...
export const FormSpecialPrice = ({
  ...
  const [formData, setFormData] = useState<SpecialPrice>(specialPrice);
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      <div className="">
        ...
        <input
          ...
          defaultValue={specialPrice.discountPercentaje}
      ...
      <div className="">
        ...
        <select
          ...
          name="user_id"
          required
          defaultValue={String(specialPrice.productId)}
          onChange={(e) => handleChange(e, setFormData)}
      ...
      >
        {users.map((user) => (
          <SelectItem key={user._id}>{user.email} </SelectItem>
        ))}
      </Select>
      ...
      <div className="w-full max-w-xl flex flex-row gap-4">
        <DatePicker
          ...
          defaultValue={parseDate(
            new Date(specialPrice.validUntil).toISOString().split("T")[0]
          )}
          label="Creado el"
          variant="bordered"
          onChange={(e) => handleChange(e, setFormData)}
